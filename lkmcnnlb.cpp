...
struct event
{
    ...
    event (list<platelet>::iterator address_, plint type_, T tau_) :
        address(address_), type(type_), tau(tau_)
    {}
};
...
T nnOutput(platelet & plt, Array<T,8> nnHistory)
{
    ...
    if (c[0][0] + c[1][0] + c[2][0] + c[3][0] + c[4][0] + c[5][0] < -5.9999)
    {
	return -1.;
    }     
...
void setBindingRates (list<platelet>::iterator current)
{
    ...
    {
        rate = rate + k_att_col * plt.activation;
    }
    ...
    {           
        for (tempHead = overlapLinks.begin(); tempHead != overlapLinks.end(); tempHead++)
        {
            ...
            if (tempPlt.bound)
            {
                rate = rate + k_att_fib * sqrt(plt.activation * tempPlt.activation * plt.activationx * tempPlt.activationx)
                    *sqrt(enhancement) * sqrt(tempEnhancement);
            }
        }
    }
...
void setUnbindingRates (list<platelet>::iterator current)
{
    ...
    {           
        ...
        for (tempHead = overlapLinks.begin(); tempHead != overlapLinks.end(); tempHead++)
        {
            ...
            tempRate = k_det_fib / (sqrt(plt.activation * tempPlt.activation * 
            plt.activationx * tempPlt.activationx));
...
bool liesInside (Array<plint,3> pos, Array <plint,3> center, T dia)
{
    ...
    return pow(realPos[0] - realCenter[0], 2.) + pow(realPos[1] - realCenter[1], 2.) + 
        pow(realPos[2] - realCenter[2], 2.) <= 0.25*dia*dia;
}
